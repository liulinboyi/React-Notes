...
/**
 * 异步任务优先级比较低
 * 它可以被打断，因为可以被打断，为了防止异步任务一直被打断，一直不能执行
 * react给这个任务设置了expirationTime，在这个时间之前你都可以打断我，
 * 如果在某个时间点，发现已经过了这个时间了，还没有被执行，就会强制执行
 * 这个异步任务，这就是expirationTime的意义
 */

/**
 * expirationTime种类：
 * 在react里面不止有异步expirationTime的存在 ，还有
 * 
 * 1. Sync模式下面的expirationTime，优先级最高的，这个任务创建完成之后
 * 立马就要更新到真正的DOM里面，创建立即更新的流程
 * 2. 异步模式，会进行调度，会有一系列复杂的操作，可能会被中断
 * 会有一个计算出来的过期时间，然后根据优先级的情况进行处理
 * 3. 指定context的情况
 * 
 */
...
function msToExpirationTime(ms) {
  ...
  return (ms / UNIT_SIZE /*10*/ | 0 /*向下取整*/) + MAGIC_NUMBER_OFFSET /*2*/;
}
...
function ceiling(num, precision) {
  return ((num / precision | 0) + 1) * precision /*精密度*/;
}

function computeExpirationBucket(currentTime, expirationInMs, bucketSizeMs) {
  /*
  最终公式是：((((currentTime - 2 + 5000 / 10) / 25 ) | 0) + 1) * 25
  其中 (currentTime - 2 + 5000 / 10)这部分是固定的，设它为X
  ((currentTime - 2 + 5000 / 10) / 25 ) => (X / 25 ) 固定值除以25
  (((currentTime - 2 + 5000 / 10) / 25 ) | 0) => ((X / 25) | 0) 固定值除以25，和0进行与操作，进行向下取整 重要，其实当两个数之间差值小于25时，同时除以25并将向下取整，结果是一样的
  ((((currentTime - 2 + 5000 / 10) / 25 ) | 0) + 1) => (((X / 25) | 0) + 1) 固定值除以25，和0进行与操作，进行向下取整，然后加1
  ((((currentTime - 2 + 5000 / 10) / 25 ) | 0) + 1) * 25 => (((X / 25) | 0) + 1) * 25
  最后的效果就是，两个数字，差值小于25，则结果相同
   让多个setstate之间，假如时间差距很小，算出的ExpirationTime相同，任务优先级相同
  */
  return MAGIC_NUMBER_OFFSET + ceiling(currentTime - MAGIC_NUMBER_OFFSET + expirationInMs / UNIT_SIZE, bucketSizeMs /*差值小于bucketSizeMs / UNIT_SIZE，则结果相同*/ / UNIT_SIZE);
}
...
function createFiberRoot(containerInfo, isConcurrent, hydrate) {
  ...
  if (enableSchedulerTracing) {
    root = {
      // 任何一个reactElement节点都会对应一个fiber对象
      // reactElement是一个树结构，fiber对象也是有一个树结构，current是这个树结构的顶点
      current: uninitializedFiber, // root节点对应的Fiber对象，root节点也是有更新的，所以需要一个fiber对象和它对应
      containerInfo: containerInfo, // root节点 render方法接收的第二个参数
      pendingChildren: null, // 只有在持久更新会用到，即不支持增量更新的平台（biruserverrender中会用到，因为需要输出很多字符串类型的html节点的内容），在react-dom中不会被用到
      // 后续任务调度过程中，用来记录各种更新所涉及的的时间的
      // 这几个变量就是，标记不同优先级任务
      ...
      didError: false, // 整个应用在渲染过程中，是否有错误
      // 正在等待提交任务的ExpirationTime
      pendingCommitExpirationTime: NoWork,
      finishedWork: null, // 在一次渲染更新过程中，完成了的更新任务 在整个树当中存在不同的更新任务，每次更新会渲染优先级最高的那个任务，当优先级最高的任务渲染完成之后，它就是finishedWork，标记在应用的fiberRoot上面。应用更新后会把应用输出到DOM上面，输出的过程就是读取的finishedWork属性，在commit阶段。
      timeoutHandle: noTimeout,
      context: null,
      pendingContext: null,
      hydrate: hydrate, // 应用是否和原来节点合并的标志
      nextExpirationTimeToWorkOn: NoWork, // 这次更新渲染时，要执行的是哪个优先级的任务 记录优先级最高的ExpirationTime，更新过程中，会根据这个变量进行更新
      expirationTime: NoWork,
      firstBatch: null,
      nextScheduledRoot: null, // 单向链表的属性
...
function createUpdate(expirationTime) {
  return {
    expirationTime: expirationTime, // 这次创建更新的过期时间

    tag: UpdateState, // tag有4中情况 UpdateState:0; ReplaceState:1; ForceUpdate:2; CaptureUpdate:3;
    payload: null, // 实际执行操作的内容
    callback: null,
...
function appendUpdateToQueue(queue, update) {
  ...
  // 最后一个update不存在，说明UpdateQueue是空的
  if (queue.lastUpdate === null) {
    // Queue is empty
    queue.firstUpdate = queue.lastUpdate = update;
  } else {
    // 如果lastupdate存在，新加的update就放在最后了
    // last newlast => last -> newlast => last = newlast
    var last = queue.lastUpdate;
    last.next = update;
    // 更新最后一个
...
function enqueueUpdate(fiber, update) {
  ...
  var alternate /*workInprogress*/ = fiber.alternate;
  var queue1 = void 0;
  var queue2 = void 0;
  if (alternate === null) {
    // 初次渲染
    ...
    if (queue1 === null) {
      if (queue2 === null) {
        // 都是null说明本身这个节点没有产生过更新，没有updatequeue，那就创建新的
  ...
  // 做这么多，就是让fiber.updateQueue和alternate.updateQueue有相同的第一层firstUpdate和lastUpdate
  // queue1 = fiber.updateQueue;
  // queue2 = alternate.updateQueue;
...
function resetStack() {
  if (nextUnitOfWork /*记录遍历子树时候，执行到了哪个节点的更新，即下一个即将更新的节点*/ !== null) {
    // 意味着之前更新的是一个异步任务，但是由于时间片不够了，就把执行权交给了浏览器，nextUnitOfWork里面保存着下一个更新的节点
    var interruptedWork = nextUnitOfWork.return; // 中断的任务
    while (interruptedWork !== null) {
      // 向上去找，所有被打断的任务
      unwindInterruptedWork(interruptedWork); // 退回更新
...
function computeExpirationForFiber(currentTime, fiber) {
  ...
  if (expirationContext !== NoWork) {
    // 一般用于，在外部强制某个更新必须使用那种expirationTime的行为 比如flushSync
    // An explicit expiration context was set;
    expirationTime = expirationContext;
  } else if (isWorking /*任务正在更新*/) {
      if (isCommitting$1) {
    ...
    if (fiber.mode & ConcurrentMode) {
      // 判断fiber.mode的模式是否是ConcurrentMode
      // 如果处于ConcurrentMode则选择两种方法中的一个去计算一个expirationTime
      ...
      if (nextRoot !== null && expirationTime === nextRenderExpirationTime) {
        // 加1是为了区分，下一个即将进行的更新和当前正要创建的更新，为了防止它们是一样的
        expirationTime += 1;
      }
    } else {
      // 如果没有处于ConcurrentMode组件渲染的子树当中，都是同步的更新
...
/**
 * 
 * @param {*} fiber 产生更新的Fiber
 * @param {*} expirationTime 当前fiber更新产生的优先级
 * @returns fiberRoot
 */
// 功能是，根据fiber节点向上找，找到rootFiber对象，当然找的过程当中还会更新所有子树的优先级
function scheduleWorkToRoot(fiber, expirationTime /*当前fiber更新产生的优先级*/) {
  recordScheduleUpdate(); // 记录更新流程的时间的
  ...
  // 更新 当前fiber（有更新）的优先级
  if (fiber.expirationTime === NoWork /*说明这个节点没有任何更新操作*/ ||
  // 更新完成后，会把expirationTime去掉，但是当前有expirationTime说明没有更新完成
  // expirationTime小则优先级高
  fiber.expirationTime > expirationTime /*说明fiber节点的更新优先级低于当前的更新*/
  ) {
      fiber.expirationTime = expirationTime;
    }
  // 获取alternate
   ...
   * 当前fiber的父节点
   */
  var node = fiber.return; // 当前fiber的父节点
  /**
   * fiberRoot
   */
  var root = null;
  if (node === null /*当前fiber的父节点位null，说明当前fiber就是rootFiber*/ && fiber.tag === HostRoot /*rootFiber*/) {
      root = fiber.stateNode; // 赋值fiberRoot给root（rootFiber的stateNode属性是fiberRoot）
    } else {
    while (node !== null) {
      alternate = node.alternate;
      if (node.childExpirationTime /**子树中优先级最高的expirationTime*/ === NoWork || node.childExpirationTime > expirationTime /*当前的更新优先级对子树来说是不是优先级更高的任务*/
      ) {
          node.childExpirationTime = expirationTime; // 当前的更新优先级对子树来说是优先级更高的任务，则更新子树优先级
          if (
          /*对alternate进行同样的更新 */
          alternate !== null && (alternate.childExpirationTime === NoWork || alternate.childExpirationTime > expirationTime)) {
      ...
      if (node.return === null && node.tag === HostRoot) {
        root = node.stateNode; // 赋值fiberRoot给root（rootFiber的stateNode属性是fiberRoot）
        break;
      }
      node = node.return;
    }
...
function scheduleWork(fiber, expirationTime) {
  ...
  if (!isWorking /*有任务正在进行当中*/ && nextRenderExpirationTime !== NoWork /*我们的任务可能是一个异步的任务，可能执行到一半了，没有执行完，
                                                                      现在要把执行权交给浏览器，去执行一些更高优先级的任务的时候，产生了一个更新*/ && expirationTime < nextRenderExpirationTime // 当前产生更新的优先级高于正在执行的操作，这就是react16提供的 非常重要的特性，可以优先执行高优先级的任务
  ) {
      // This is an interruption. (Used for performance tracking.)
      interruptedBy = fiber; // 仅用来记录，主要在devtool里面有作用
      resetStack(); // 新的高优先级任务，打断了老的低优先级的任务的执行
    }
  markPendingPriorityLevel(root, expirationTime);
  if (
  // If we're in the render phase, we don't need to schedule this root
  // for an update, because we'll do it before we exit...
  // working和 commiting是react的渲染的不同阶段，working包含render和commiting
  // commiting是第二阶段，是不可打断的阶段 fiber树整体渲染完成之后，一个新的状态出来之后，更新到DOM上的过程
  !isWorking /*没有正在工作*/ || isCommitting$1 /*正在提交，说明上一次的更新已经结束了，所以可以继续去请求工作的*/ ||
  // ...unless this is a different root than the one we're rendering.
  nextRoot /*上一次要进行更新的root*/ !== root /*对于单页面应用来说，nextRoot===root会一直成立*/
  ) {
      var rootExpirationTime = root.expirationTime;
      requestWork(root, rootExpirationTime);
    }
  // 防止在ComponentDidMount里面修改setState，导致又进行了一次更新，ComponentDidMount又会再执行，
  // 导致无限循环
...
function recomputeCurrentRendererTime() {
  ...
  // debugger
  currentRendererTime = msToExpirationTime(currentTimeMs) /*将currentTimeMs除以10向下取整然后加2*/;
}

function scheduleCallbackWithExpirationTime(root, expirationTime) {
  if (callbackExpirationTime !== NoWork) {
    // 上次的回调已经被调度了，检查它的过期时间
    // A callback is already scheduled. Check its expiration time (timeout).
    if (expirationTime > callbackExpirationTime) {
      // expirationTime比callbackExpirationTime大说明，expirationTime优先级低
      // 存在的回调有足够的时间，则退出
      // Existing callback has sufficient timeout. Exit.
      return;
    } else {
      // expirationTime比callbackExpirationTime小说明，expirationTime优先级高
      if (callbackID !== null) {
  ...
  // currentMs是从react加载进来后，获取到的时间和当前时间的差
  var expirationTimeMs = expirationTimeToMs(expirationTime); // 算出expirationTime转换成MS的值
  // expirationTimeMs比当前时间要久的，是算出将来某个时间点认为这个任务已经过期了，一般不会比当前时间要小，如果小，表示已经过期了，要立马执行
  var timeout = expirationTimeMs - currentMs; // 算出一个超时时间
  // scheduleDeferredCallback这个方法来自于scheduler包
  callbackID = unstable_scheduleCallback(performAsyncWork, { timeout: timeout }); // 返回一个callbackId，可以帮助我们cancel这个callback
}
...
function requestCurrentTime() {
  ...
  if (isRendering /*进入到渲染阶段了*/) {
      // We're already rendering. Return the most recently read time.
      return currentSchedulerTime;
    }
  // Check if there's pending work.
  // 从调度队列当中找到一个权限最高的root
  findHighestPriorityRoot();
  if (
  // 最开始没有任何调度的任务，这两个值是默认的
  nextFlushedExpirationTime === NoWork || nextFlushedExpirationTime === Never) {
...
function requestWork(root, expirationTime) {
  ...
  // render过程中，react会去循环firstsceduleroot和lastscheduleroot上的任务
  // 最终把所有更新，更新完为止，然后把firstsceduleroot和lastscheduleroot置为null
  if (isRendering /*判断正在渲染 说明react的渲染已经在做了，循环已经开始了，那就直接return*/) {
      // Prevent reentrancy. Remaining work will be scheduled at the end of
      // the currently rendering batch.
      return;
    }

  if (isBatchingUpdates /*批处理*/) {
      // Flush work at the end of the batch.
      if (isUnbatchingUpdates /*批处理*/) {
  ...
  if (expirationTime === Sync /*判断expirationTime是否等于sync*/) {
      performSyncWork(); // 同步执行JavaScript代码，一直执行到结束为止，无法被打断
    } else {
    // 进行异步的调度
    // 调用独立的npm包，scheduler
    // 在浏览器空闲的情况下，执行任务
    // 并且有deadline，在这个时间前可以执行，在这个时间后，必须把执行权交给浏览器，等再有空闲来执行任务
...
function addRootToSchedule(root, expirationTime) {
  ...
  // 如果只有一个root（单页面应用）则root.nextScheduledRoot === root
  if (root.nextScheduledRoot === null) {
    // 说明这个root之前没有进入过调度
    // This root is not already scheduled. Add it.
    root.expirationTime = expirationTime;
    // 全局变量firstScheduleRoot和lastScheduledRoot是一个单向链表结构，用来存储react当中的所有root的调度关系
    // 如果有多个root存在的情况下，形成一个链表的结构
    if (lastScheduledRoot === null) {
      firstScheduledRoot = lastScheduledRoot = root;
      root.nextScheduledRoot = root;
    } else {
      // 把当前root插到当前队列（单向链表）的最后一个
      lastScheduledRoot.nextScheduledRoot = root;
      lastScheduledRoot = root;
      lastScheduledRoot.nextScheduledRoot = firstScheduledRoot;
    }
  } else {
    // 如果原来进入过调度，则先获取expirationTime
    // This root is already scheduled, but its priority may have increased.
    var remainingExpirationTime = root.expirationTime;
    if (remainingExpirationTime === NoWork || expirationTime < remainingExpirationTime) {
      // Update the priority.
      // 增加优先级
      // root.expirationTime必须是这个root上面优先级最高的更新，不能是优先级低的，我们会优先执行优先级高的任务，后续会执行优先级低的任务。
      root.expirationTime = expirationTime;
    }
  }
}
...
// react的事件产生的更新，事件的回调函数，执行的时候，大部分都是isBatchingInteractiveUpdates为true
...
function scheduleRootUpdate(current$$1, element, expirationTime, callback) {
  ...
  // 把update加入到Fiber对应updatequeue的里面
  // react整体上会存在多次更新在一个节点上产生
  enqueueUpdate(current$$1, update);
  // 开始进行任务调度 告诉react，我们有更新产生了，我们要进行更新了，进行调度了
  // 在react16提供了一个任务优先级的概念，有可能在同一时间，有不同优先级的任务在应用里面，需要调度执行优先级高的任务，再执行优先级低的任务。
...
function updateContainerAtExpirationTime(element, container, parentComponent, expirationTime, callback) {
  ...
  // 获取了一个context，目前这个版本的context拿不到，因为parentComponent是一个null
  var context = getContextForSubtree(parentComponent);
  if (container.context === null) {
    container.context = context; // 目前认为没有
  } else {
    container.pendingContext = context; // 目前认为没有
  }
...
function updateContainer(element, // <App />
container, // FiberRoot
parentComponent, callback) {
  var current$$1 = container.current;
  var currentTime = requestCurrentTime();
  // 这个是能够使用concurrentMode进行优先级的任务更新非常重要的一个点
  // 终止时间
  var expirationTime = computeExpirationForFiber(currentTime, current$$1);
  // 更新容器在截止时间
  return updateContainerAtExpirationTime(element, // <App />
  container, // FiberRoot
...
function ReactRoot(container, isConcurrent, hydrate) {
  ...
  // 遍历Fiber方法
  // 当前fiber如果有子节点的话，会在child属性上
  // 如果有多个子节点，那么所有子节点的第一个子节点会作为child
  // 其他子节点会作为sibling属性

  // 如果进行节点的查找，就从root通过child一路向下找，直到找到一个叶子结点，
  // 恰好子节点都会指向父节点（return）（每个节点的父节点只有一个，不会有多个）
  // 然后判断当前节点是否有兄弟节点，如果有兄弟节点，就会往兄弟节点遍历，
  // 一直遍历，发现没有兄弟节点和子节点了，返回父节点，父节点也没有兄弟节点了，返回父节点，最终不断返回，直至遍历完整棵树。
  // 通过react这种数据结果普，把fiber整个串联起来。提供了一个方便的遍历方式，以一种高效率的方式遍历完。

  var root = createContainer(container, isConcurrent, hydrate);
  this._internalRoot = root;
}
ReactRoot.prototype.render = function (children, // <App />
callback) {
  ...
  // 最终调用了这个
  updateContainer(children /*<App />*/, root /*FiberRoot*/, null, work._onCommit);
  return work;
};
...
function shouldHydrateDueToLegacyHeuristic(container) {
  ...
  return !!(rootElement && // 是否存在
  rootElement.nodeType === ELEMENT_NODE && // 是否是普通的element type
...
function legacyRenderSubtreeIntoContainer(parentComponent, children, // <App />
container, forceHydrate, callback) {
  ...
  var root = container /*我们传入的Dom节点*/._reactRootContainer;
  if (!root) {
    // DOM节点上_reactRootContainer不存在的情况
    // Initial mount
    // 加上这个_reactRootContainer属性
    // 创建root
    ...
    // 不批量更新
    // 可以认为是直接执行了一个回调，这个回调是直接执行的
    unbatchedUpdates(function () {
      if (parentComponent != null) {
        root.legacy_renderSubtreeIntoContainer(parentComponent, children, callback);
      } else {
        root.render(children /*<App />*/, callback);
      }
    });
  } else {
    if (typeof callback === 'function') {
...
var ReactDOM = {
  ...
  // render方法
  render: function (element, /*ReactElement <App />*/
  // 每个reactElement都对应一个Fiber对象，会记录节点的各种状态 比如customCpmponent它的state和props，这些是记录在fiber对象上面的，
  // 在fiber更新之后才会更新到customConponent上面的this.state和this.props里面,这些操作都是在fiber上面进行操作的，这样也给了react能够做hooks的方便
  // fiber个重要作用，记录节点的各种状态
  // 串联整个应用，形成树结构
  container, /*挂载到哪个节点上面*/
  callback) /*应用渲染结束后会调用这个callback*/
  {
    // 遗传给子树进入容器
    return legacyRenderSubtreeIntoContainer(null, // null
    element, // <App />
